...
export default function TestPage() {
  ...
  useEffect(() => {
    const fetchHistory = async () => {
      ...
      try {
        ...
        const formatted = history.map((m: { role: string; message: string; created_at?: string }) => ({
          role: m.role as Role,
          text: m.message,
          ts: m.created_at
  ...
  return (
    <div className="flex flex-col h-screen bg-[#D9F3F3] dark:bg-[#0B1120] max-w-7xl mx-auto px-4 py-8 space-y-6">
      {/* HEADER */}
      <div className="flex items-center justify-between p-4 border-b border-[#B8E0E0] dark:border-zinc-700 bg-white dark:bg-zinc-900">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              <h1 className="font-bold text-[#0B3C3C] dark:text-white">Test Chatbot</h1>
              <p className="text-xs text-[#2F6F6F] dark:text-zinc-400">ID: {chatbotId}</p>
            </div>
          </div>
        </div>
        <button
        ...
        {messages.map((msg, i) => (
          ...
          >
            ...
            >
              ...
              <span className={`text-[10px] mt-1 block ${msg.role === "user" ? "text-white/70" : "text-[#2F6F6F] dark:text-zinc-400"}`}>
                {msg.ts}
              </span>
            </div>
          </div>
      ...
      {/* INPUT */}
      <div className="p-4 border-t border-[#B8E0E0] dark:border-zinc-700 bg-white dark:bg-zinc-900">
