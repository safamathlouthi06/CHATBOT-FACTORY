...
export default function CreateChatbotPage() {
  ...
  const goToStep = (step: number) => {
    if (step === 1) {
      router.push("/dashboard/chatbots/create?step=1");
      return;
    }
    ...
    router.push(`/dashboard/chatbots/create?id=${activeChatbotId}&step=${step}`);
  };
...
if (!res.ok) {
  ...
  return;
      ...
      router.push(`/dashboard/chatbots/create?id=${newId}&step=2`);
  ...
  const handleSaveKnowledge = async () => {
    ...
    try {
      ...
      router.push(`/dashboard/chatbots/create?id=${activeChatbotId}&step=3`);
    } catch (err: any) {
      console.error(err);
